...
class HTTPClientManager:
    ...
    async def get_session(
        self,
    ) -> aiohttp.ClientSession:  # Get or create session with double-checked locking pattern !!!
    ...
    ):  # Create HTTP session with performance optimized configuration !!!
    ...
    def _create_timeout() -> aiohttp.ClientTimeout:  # Create timeout configuration for HTTP requests !!!
        return aiohttp.ClientTimeout(total=10, connect=2, sock_read=8)

    @staticmethod
    def _create_headers() -> Dict[str, str]:  # Create default HTTP headers for requests !!!
        return {"User-Agent": "PuzzleSolver/2.0", "Accept": "application/json", "Connection": "keep-alive"}
    ...
    ) -> AsyncGenerator[aiohttp.ClientResponse, None]:  # HTTP request context manager with error handling !!!
        try:
    ...
    async def fetch_json(
        self, url: str, **kwargs
    ) -> Optional[Dict[str, Any]]:  # Optimized JSON fetch helper with comprehensive error handling !!!
        try:
