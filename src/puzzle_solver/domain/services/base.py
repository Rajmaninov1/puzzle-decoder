...
class BaseWebService(ABC, Generic[T]):
    ...
    @abstractmethod
    def parse_response(self, data: Dict[str, Any]) -> Optional[T]:
    ...
    async def fetch_single(self, **params: Any) -> Optional[T]:
        """Fetch single item."""
    ...
    async def _fetch_with_retry(self, **params: Any) -> Optional[T]:
    ...
    async def fetch_batch(self, param_list: List[Dict[str, Any]]) -> List[T]:
    ...
    async def fetch_range(self, start: int, count: int, param_builder: Callable[[int], Dict[str, Any]]) -> List[T]:
        """Fetch a range of items."""
        param_list = [param_builder(start + i) for i in range(count)]
        return await self.fetch_batch(param_list)

    def _filter_valid_results(self, results: List[Any]) -> List[T]:
        """Filter valid results from batch fetch."""
