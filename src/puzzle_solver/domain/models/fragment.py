...
class Fragment(BaseModel):
    ...
    def __str__(self) -> str:  # String representation with truncated text for readability !!!
        return (
            f"Fragment({self.id}, idx={self.index}, text='{self.text[:20]}...')"
            if len(self.text) > 20
            else f"Fragment({self.id}, idx={self.index}, text='{self.text}')"
        )
...
class FragmentBatch(BaseModel):
    ...
    def completion_percentage(
        self,
    ) -> float:  # Calculate completion percentage based on found vs expected fragments !!!
